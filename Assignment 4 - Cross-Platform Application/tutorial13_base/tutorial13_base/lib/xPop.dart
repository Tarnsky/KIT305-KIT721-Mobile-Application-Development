...
class xPop extends StatefulWidget {
  xPop({Key? key, required this.title, required this.repLimit})
      : super(key: key);
  ...
  @override
  _xPop createState() => _xPop();
}

class _xPop extends State<xPop> {
  int? scoreLimit = _xPop.repLimit;
  int score = 0;
  int totalClicks = 0;
  ...
  Widget build(BuildContext context) {
    var numberList = ['O', 'O', 'X'];
    numberList.shuffle();
    var buttonA = numberList[0];
    var buttonB = numberList[1];
    var buttonC = numberList[2];
      ...
      body: Center(
        child: SingleChildScrollView(
          child: Column(
            children: <Widget>[
              Text('xPop'),
              Text('Press The X button to score'),
              Text('Score: ' + '$score'),
              SizedBox(height: 30),
              Container(
                height: sizeHight,
